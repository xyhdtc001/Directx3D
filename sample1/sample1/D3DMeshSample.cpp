...
static void dumpVertices(std::ofstream& outFile, ID3DXMesh* mesh);
static void dumpIndices(std::ofstream& outFile, ID3DXMesh* mesh);
static void dumpAttributeBuffer(std::ofstream& outFile, ID3DXMesh* mesh);
static void dumpAdjacencyBuffer(std::ofstream& outFile, ID3DXMesh* mesh);
static void dumpAttributeTable(std::ofstream& outFile, ID3DXMesh* mesh);
...
D3DMeshSample::D3DMeshSample(IDirect3DDevice9* pDevice)
	:CSampleBase(pDevice)
{
}


D3DMeshSample::~D3DMeshSample()
{
}

bool D3DMeshSample::setup()
{
	HRESULT hr = 0;
	...
	D3DXCreateTextureFromFile(
		Device,
		_T("dx_res\\brick0.jpg"),
		&Textures[0]);

	D3DXCreateTextureFromFile(
		Device,
		_T("dx_res\\brick1.jpg"),
		&Textures[1]);

	D3DXCreateTextureFromFile(
		Device,
		_T("dx_res\\checker.jpg"),
		&Textures[2]);
...
bool D3DMeshSample::display(float timeDelta)
{
	...
	_BASE_RENDER_FPSCOUNT(pFontMesh,10,10,300,40,(d3d::WHITE));
	m_device->EndScene();
	m_device->Present(0, 0, 0, 0);
	return true;
}

bool D3DMeshSample::cleanup()
{
	__super::cleanup();
	return true;
}
